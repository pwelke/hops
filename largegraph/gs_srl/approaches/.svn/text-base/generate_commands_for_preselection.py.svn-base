...
def create_worker_PBS_script_sampling(pbs_script_folder,worker_script_exhaustive_file,path_to_running_script,pattern_path,experiment_name,approach_name_script,selected,pattern_level_path,select_on): 
    #global wall_time_string   
    #date_object = datetime.strptime(wall_time_string, '%H:%M:%S')
    #time=date_object.time()
    nr_commands=get_nr_commands_in_worker_script(worker_script_exhaustive_file)
...
def main(data_graph_path,pattern_path,pattern_level_path,output_path_arg,output_script,path_to_scripts,level,runs,time_interval,max_time,selected,select_on):
    ...
    create_worker_PBS_script_sampling(worker_scripts_path,sampling_data_worker_script_path,path_to_scripts,pattern_path,"fur_"+str(level),"furer_sampling_approach.py",selected,pattern_level_path,select_on)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-select_on',type=int)
    
    args = parser.parse_args()
    if args.level==None:
        raise Exception('specify the level')
    print "Generating commands for preselection"
    main(args.data_graph_path,args.pattern_path,args.pattern_level_path,args.output_path,os.path.join(args.output_script,"presampling"),args.path_to_scripts,args.level,args.runs,args.time_interval,args.max_time,args.selected,args.select_on)
