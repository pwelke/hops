...
if __name__ == '__main__':
    ...
    o=0;
    for n in Plist_base:
       if pattern.node[n]['target'] == True:
         target_node_order[n]=o;
         o+=1;   
    orderings=u.get_all_random_orderings(pattern, hist)
    

    #the rest of basic orderings are just permutations of their lists
    N=10;
    random.shuffle(orderings)
    selected_orderings=orderings[0:N]
    print "nr of randomly selected orderings: ",len(selected_orderings)

    
    #Choose up to 10 random ordering
    Plist_dict={}
    OBdecomp_false_dict={}
    #selected_orderings=[[2,1,0,3,4]]
    counter=1
    for o in selected_orderings:
       print o
       OBdecomp_false = [[item] for item in o]
       Plist = o
    ...
    for i in xrange(1,len(OBdecomp_false_dict)+1):
        ...
        experiments.globals.sum_number_of_embeddings=0
        experiments.globals.sum_of_the_square_embeddings=0
        experiments.globals.sum_number_of_extra_embeddings=0
    ...
    average_time=average_time/len(OBdecomp_false_dict)
    ...
    report.main_1(args.output_path,args.data_graph_path,args.redo,'results_false_furer_order_random',monitoring_reports)
