...
if __name__ == '__main__':
    ...
    parser.add_argument('-max_time',type=int,help='path to data file')
    ...
    fudicts = []
    no_obd_decomp=False
    #short_graph_file_name=args.short_graph_name
...
#     
    ...
    for i in range(args.runs):
        ...
        if no_obd_decomp:
          p=false_furer.False_Furer(data_graph,  pattern,  OBdecomp,  root_nodes,NLIMIT_values,output_path,lock,i,nr_embeddings)
        else:
          p=furer.Furer(data_graph,  pattern,  OBdecomp,  root_nodes,NLIMIT_values,output_path,lock,i,nr_embeddings)
        m=monitor_process.process_monitor.monitor_process_furer(p,monitoring_marks,lock,i,nr_nodes_data_graph,args.select_on,args.sample,os.path.join(pattern_level_results,'selected_patterns.info'),args.cutoff,output_path)
        ...
        if args.sample:
            ...
            if m.successful_monitoring==False:
                ...
                nr_emb_exh=None
                nr_emb_exh=experiments.exhaustive_approach.get_nr_emb_within_time(args.data_graph_path,args.pattern_path, args.output_path,120);
                with open(os.path.join(args.output_path,'test_exhaustive_120_seconds.info'),'w') as f:
                    f.write(str(nr_emb_exh))
    ...
    report.main(output_path,args.data_graph_path)
