...
import report_results.false_furer_report_orderings as report
import argparse
...
if __name__ == '__main__':
    ...
    output_path=os.path.join(args.output_path,"results_false_furer_order")
    ...
    OBdecomp_base = OBDsearch.get_heuristic4_OBD(pattern, startNode = root_node)
    OBdecomp_false_base=None
    if OBdecomp_base!=None:
       OBdecomp_false_base = [[item] for sublist in OBdecomp_base for item in sublist]
    else:
       OBdecomp_false_base =OBDsearch.get_flatList(pattern, startNode=root_node)
    print "OBD: ",OBdecomp_false_base
    Plist_base = [item for sublist in OBdecomp_false_base for item in sublist]
    
    
    basic_obdecomps=u.get_all_obds(pattern, hist)
    print basic_obdecomps
    while True:
        #the rest of basic orderings are just permutations of their lists
        shuffle_list=copy.deepcopy(list(random.choice(basic_obdecomps)))
        print "List to shuffle: ",shuffle_list
        #randomly shuffle one of the arrays of the list having >1 elements in the list
        indices=[]
        for el in shuffle_list:
            if len(el)>1:
                indices.append(shuffle_list.index(el))
        
        index_to_shuffle=random.choice(indices)
        print "Index to shuffle: ",index_to_shuffle,"and list: ",shuffle_list[index_to_shuffle]
        random.shuffle(shuffle_list[index_to_shuffle])
        print "Shuffles: ",shuffle_list
        basic_obdecomps.append(shuffle_list)
        if len(basic_obdecomps)==10:
            break
    
    print "All obdecomps!",basic_obdecomps
    print len(basic_obdecomps)   
    #Choose up to 10 random ordering
    Plist_dict={}
    OBdecomp_false_dict={}
    
    counter=1
    for o in basic_obdecomps:
       OBdecomp_false = [[item] for sublist in o for item in sublist]
       Plist = [item for sublist in OBdecomp_false for item in sublist]
       Plist_dict[counter]=Plist
       OBdecomp_false_dict[counter]=OBdecomp_false
       counter+=1
           
    print Plist_dict
    print OBdecomp_false_dict
    plot_result_dict = {}
    monitoring_reports={}
    average_time=0
    
    print "monitoring marks: ",monitoring_marks
    for i in xrange(1,len(OBdecomp_false_dict)+1):
        print "-------------- Run:-------------",i
        print "ordering: ",OBdecomp_false_dict[i]
        lock = threading.Lock()
        start=time.time() 
        ...
        m=monitor.monitor_process_false_furer(p,monitoring_marks,lock,i,nr_nodes_data_graph,-1,False)
    ...
    pickle.dump(Plist_dict, pickout)
    pickout.close()
    
    #write down Plist
    pickout = open(os.path.join(output_path,'Plist_base.pickle'), 'wb')
    pickle.dump(Plist_base, pickout)
    pickout.close()
    ...
    report.main(args.output_path,args.data_graph_path,args.redo)
