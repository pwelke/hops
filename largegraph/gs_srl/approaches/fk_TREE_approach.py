...
import time
from sampling_core import tree_sampling_algorithm as tree
...
def get_nr_embedding(data_graph, pattern, root_node, root_node_predicate_name, monitoring_marks):
    ...
    lock = threading.Lock()
    p = tree.Furer(data_graph, pattern, root_node, root_nodes, None,lock,0,0)
...
if __name__ == '__main__':
    ...
    data_graph, pattern, _, root_node, root_node_predicate_name, interval, max_time, monitoring_marks, root_nodes, Plist= prepare_inputs.prepare_params(args)
    ...
    m, estimates = get_nr_embedding(data_graph, pattern, root_node, root_node_predicate_name, monitoring_marks)
