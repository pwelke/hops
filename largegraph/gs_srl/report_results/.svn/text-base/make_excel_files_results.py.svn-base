...
def create_workbook(csv_path,level):
    ...
    with open(os.path.join(csv_path,'furer_'+str(level)+'.csv'), 'rb') as f:
        ...
        for r, row in enumerate(reader):
            ...
            if r>0:
               ...
               counter=0
               print 'Exhaustive embeddings size: ',str(len(embeddings_exhaustive))
    ...
    for i in xrange(0,len(embeddings_exhaustive)):
            absError_furer=None
...
if __name__ == '__main__':
    ...
    parser.add_argument('-csv', help='path to result csvs (will also be an output for the excel file)')
    parser.add_argument('-level', help='pattern_level')
    args = parser.parse_args() 
    create_workbook(args.csv,args.level)
