...
Works for the two NCI*. There, positions
of vertex labels and incidence matrix are exchanged. I love it.
Use ShervashidzeNCI2aids.py instead. Also, labeling differs, thus
here, we replace 0 by -1.'''
...
def transform(filename, dataname, labelname, out, err):
	...
	for i in range(graphs.shape[1]):
		...
		if graphLabel == 0:
			graphLabel = -1
		vertexLabels = graphs[0,i][0]
		try:
			m = graphs[0,i][1].nnz / 2
			edges = graphs[0,i][1].todense()
		except:
			edges = graphs[0,i][1]
			m = (edges > 0.0).sum() / 2
