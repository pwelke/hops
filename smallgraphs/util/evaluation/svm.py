...
KERNEL_TYPE = ['LINEAR', 'POLY', 'RBF', 'SIGMOID', 'PRECOMPUTED', 'JACCARD', 'INTERSECTION']
...
def gen_svm_nodearray(xi, feature_max=None, isKernel=None):
	...
	# for r in ret:
	# 	sys.stderr.write(str(r.index) + ' ' + str(r.value) + ', ')
	# sys.stderr.write(str(max_idx) + '\n')
	return ret, max_idx
...
import sys
def gen_svm_nodearray_sparse(xi, isKernel=None):
	if str(type(xi)) != "<class 'scipy.sparse.csr.csr_matrix'>":
		raise TypeError('xi should be a csr_matrix but seems to be ' + str(type(xi)))
	if isKernel:
		raise TypeError('Cannot handle kernel values at the moment.')

	# ret contains a struct with s.index=-1 at last position
	ret = (svm_node * (xi.data.shape[0] + 1))()
	ret[-1].index = -1 # terminator
	max_idx = 0

	if len(ret) != 1:
		# coo is nice to traverse
		xi.sort_indices()
		x = xi.tocoo()
		max_idx = x.col[-1] + 1 # max index in this row
		for r,i,v in itertools.izip(ret[1:], x.col, x.data):
			r.index = i + 1
			r.value = v
...
class svm_problem_sparse(svm_problem):
	_names = ["l", "y", "x"]
	_types = [c_int, POINTER(c_double), POINTER(POINTER(svm_node))]
	_fields_ = genFields(_names, _types)
	...
	def __init__(self, y, x, isKernel=None):
		...
		x_space = self.x_space = []
		for i, xi in enumerate(x):
			tmp_xi, tmp_idx = gen_svm_nodearray_sparse(xi,isKernel=isKernel)
			x_space += [tmp_xi]
			max_idx = max(max_idx, tmp_idx)
		self.n = max_idx
