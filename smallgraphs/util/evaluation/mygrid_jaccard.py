...
#     5 -- jaccard: jaccard similarity on non-0 features. values ignored)
#     6 -- intersection: |{i : u[i] == v[i] and u[i] != 0}|
...
g_begin, g_end, g_step = 1,1,1 # kernel is parameter free
fold = 3
	...
	def run(self):
		while True:
			...
			try:
				...
				# we failed, let others do that
				traceback.print_exception(sys.exc_info()[0], sys.exc_info()[1], sys.exc_info()[2])
				
				self.job_queue.put((c,g))
				print('worker {0} encountered some error.'.format(self.name))
			else:
...
def main():
	...
	# create worker list
	workers = [LocalWorker('local' + str(i), job_queue, result_queue, auc_dict) for i in range(nr_local_worker)]
	# fire local workers
	...
	while (True):
		time.sleep(10)
		if ((job_queue.qsize() == 0) | sleeping(workers)):
			print auc_dict
