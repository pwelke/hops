...
void PatternTree::addExtensionLegs ( Leg &tuple ) {
  if ( tuple.depth == maxdepth ) {
    legmanager.appendExtensionLegs ( NODEPTH, graphstate.lastNode (), tuple.connectingnode, MAXEDGELABEL );
    return;
  }
  EdgeLabel minlabel = NOEDGELABEL, neglect = NOEDGELABEL, pathlowestlabel = treetuples[tuple.depth + 1 + rootpathstart].edgelabel;
  ...
  if ( nextprefixindex != NONEXTPREFIX ) {
    ...
    minlabel = treetuples[nextprefixindex].edgelabel;
  ...
  if ( minlabel == NOEDGELABEL )
    minlabel = MINEDGELABEL;
  legmanager.appendExtensionLegs ( tuple.depth + 1, graphstate.lastNode (), tuple.connectingnode, minlabel, pathlowestlabel, neglect );
}

void PatternTree::addLeftLegs ( Path &path, Leg &leg, int &i, Depth olddepth, EdgeLabel lowestlabel, int leftend, int edgesize2 ) {
  ...
  for ( ; i < path.legs.size () && path.legs[i].depth <= leftend; i++ ) {
    if ( path.legs[i].depth != olddepth ) {
      ...
      olddepth = path.legs[i].depth;
      lowestlabel = path.edgelabels[olddepth - 1];
      int i2 = i;
      while ( i2 < path.legs.size () && path.legs[i2].depth == olddepth ) {
        if ( path.legs[i2].edgelabel == lowestlabel ) {
	  Leg leg2 = path.legs[i2];
	  leg2.depth = edgesize2 - olddepth;
    ...
    if ( path.legs[i].edgelabel != lowestlabel ) {
      Leg leg2 = path.legs[i];
      leg2.depth = edgesize2 - olddepth;
      legmanager.appendLeg ( leg2 );
    }
  }
...
int PatternTree::addLeftLegs ( Path &path, Leg &leg, Leg &leg2, unsigned int legindex, int leftend, int edgesize2 ) {
  int i;
  ...
  if ( rootpathrelations.back () == 0 && leg2.depth != maxdepth ) {
    ...
    for ( i = legindex - 1; i >= 0 && path.legs[i].depth == leg.depth; i-- );
    EdgeLabel lowestlabel = path.edgelabels[leg.depth - 1];
    for ( i++; i < legindex; i++ ) {
      if ( path.legs[i].edgelabel != lowestlabel ) {
        Leg leg3 = path.legs[i];
	leg3.depth = leg2.depth;
        legmanager.appendLeg ( leg3 );
      }
    }
  }
  ...
  i = legindex + 1;
  addLeftLegs ( path, leg, i, leg.depth, path.edgelabels[leg.depth - 1], leftend, edgesize2 );

  return i;
}

void PatternTree::addRightLegs ( Path &path, Leg &leg, int &i, Depth olddepth, EdgeLabel lowestlabel, int rightstart, int nodesize2 ) {
  int i2 = i + 1, k;

  while ( i >= 0 && path.legs[i].depth >= rightstart ) {
    // we encounter a new depth, do all the extensions of the previous depths...
    if ( path.legs[i].depth != olddepth ) {
      for ( k = i + 1; k < i2; k++ ) {
        if ( path.legs[k].edgelabel != lowestlabel ) {
	  Leg leg2 = path.legs[k];
	  leg2.depth -= nodesize2;
	  legmanager.appendLeg ( leg2 );
        }
      }
      i2 = i + 1;
...
int PatternTree::addRightLegs ( Path &path, Leg &leg, Leg &leg2, unsigned int legindex, int rightstart, int nodesize2 ) {
  int i;
  ...
  i = legindex - 1;
  while ( i >= 0 && path.legs[i].depth == leg.depth )
    i--;

  // brothers of the new leg
  if ( rootpathrelations.back () == 0 && leg2.depth != maxdepth )
    for ( int j = i + 1; j < legindex; j++ ) {
      Leg leg3 = path.legs[j];
      leg3.depth = leg2.depth;
      legmanager.appendLeg ( leg3 );
    }
  EdgeLabel lowestlabel = path.edgelabels[leg.depth];
  for ( int j = legindex + 1; j < path.legs.size () && path.legs[j].depth == leg.depth; j++ ) {
    if ( path.legs[j].edgelabel != lowestlabel ) {
      Leg leg3 = path.legs[j];
      leg3.depth = leg2.depth;
      legmanager.appendLeg ( leg3 );
    }
  ...
  if ( i >= 0 )
    addRightLegs ( path, leg, i, leg.depth, lowestlabel, rightstart, nodesize2 );

  return i;
}

PatternTree::PatternTree ( Path &path, unsigned int legindex ) {
  Leg &leg = path.legs[legindex];
  
  OUTPUT(path.legs[legindex].frequency)

  maxdepth = path.edgelabels.size () / 2 - 1;
  int leftwalk, leftstart, rightwalk, rightstart;

  legmanager.appendCloseLegs ( path.closelegs );
  ...
  if ( path.totalsymmetry ||
       leg.depth * 2 == path.edgelabels.size () ) {
    while ( leftwalk >= 0 &&
    ...
    if ( leftwalk == -1 || path.edgelabels[leftwalk] < path.edgelabels[rightwalk] ) {
      ...
      for ( i = leftstart, j = 0; i >= 0; i--, j++ ) {
        vector_push_back ( Leg, treetuples, tuple );
        tuple.edgelabel = path.edgelabels[i];
        tuple.depth = j;
      }

      int leftend = nodesize2 - 1;

      if ( leg.depth <= leftend ) {
        // extension is an extension of the first path

        // add extension tuple
	Leg tuple = leg;
	tuple.depth = edgesize2 - tuple.depth;
	treetuples.push_back ( tuple );
        ...
        if ( tuple.edgelabel == treetuples[tuple.depth].edgelabel ) {
          nextprefixindex = tuple.depth + 1;
          rootpathrelations.push_back ( 0 );
        }
        else {
          nextprefixindex = NONEXTPREFIX;
          rootpathrelations.push_back ( treetuples[tuple.depth].edgelabel - tuple.edgelabel );
        }
	...
	legmanager.appendSecondPathMarker ( secondpathleg );
        ...
        if ( j >= i && path.legs[j].depth == path.edgelabels.size () ) {
          // do not include legs after the longest path!
          do
            j--;
          while ( j >= i && path.legs[j].depth == path.edgelabels.size () );
          j2 = j + 1;
        }


        if ( j >= i && path.legs[j].depth == path.nodelabels.size () - 2 ) {
          ...
          if ( path.legs[j].edgelabel >= path.edgelabels.back () ) {
            do {
              j--;
            }
            while ( j >= i && path.legs[j].depth == path.nodelabels.size () - 2
                            && path.legs[j].edgelabel >= path.edgelabels.back () );
            for ( k = j + 1; k < j2; k++ ) {
	      Leg leg2 = path.legs[k];
	      leg2.depth -= nodesize2;
	      legmanager.appendLeg ( leg2 );
	...
	treetuples.push_back ( tuple );
        ...
        rightmostindexes.push_back ( treetuples.size () - 1 );
        if ( tuple.edgelabel == treetuples[tuple.depth + rootpathstart ].edgelabel ) {
          nextprefixindex = tuple.depth + rootpathstart + 1;
          rootpathrelations.push_back ( 0 );
        }
        else {
          nextprefixindex = NONEXTPREFIX;
          rootpathrelations.push_back ( treetuples[tuple.depth + rootpathstart].edgelabel - tuple.edgelabel );
        }
	...
	treetuples.push_back ( tuple );
        ...
        while ( j <= i && path.legs[j].depth == 1
                       && path.legs[j].edgelabel < path.edgelabels[0] )
          j++;
    ...
    // add extension tuple
    Leg tuple = leg;
    tuple.depth = edgesize2 - leg.depth;
    treetuples.push_back ( tuple );
    ...
    int targetdepth = path.nodelabels.size () - 1 - leg.depth;

    j = path.legs.size () - 1;

    if ( targetdepth == path.edgelabels.size () - 1 ) {
      while ( j >= i && path.legs[j].depth > targetdepth )
        j--;
      int j2 = j;
      while ( j >= i && path.legs[j].depth == targetdepth && path.legs[j].edgelabel >= leg.edgelabel )
        j--;
      for ( int k = j + 1; k <= j2; k++ ) {
        Leg leg2 = path.legs[k];
	leg2.depth = tuple.depth;
	legmanager.appendLeg ( leg2 );
      }
      while ( j >= i && path.legs[j].depth == targetdepth )
        j--;
      ...
      while ( j >= i && path.legs[j].depth > targetdepth )
        j--;
      if ( j >= i ) {
        if ( rootpathrelations.back () != 0 && path.legs[j].depth == targetdepth ) {
          int j2 = j;
          EdgeLabel lowestlabel = path.edgelabels[targetdepth];
          while ( j >= i &&
	          path.legs[j].depth == targetdepth &&
	  	  path.legs[j].edgelabel >= tuple.edgelabel )
            j--;
          for ( int k = j + 1; k <= j2; k++ )
            if ( path.legs[k].edgelabel != lowestlabel ) {
	      Leg leg2 = path.legs[k];
  ...
  tidlist = path.tidlist->evaluateState();
...
PatternTree::PatternTree ( PatternTree &parenttree, unsigned int legindex ) {
  ...
  OUTPUT(parenttree.legs[legindex].frequency)

  legmanager.appendCloseLegs ( parenttree.closelegs );
  ...
  /*for ( int i = 0; i < treetuples.size (); i++ ) {
    fprintf ( output, "(%i,%i)", treetuples[i].depth, treetuples[i].edgelabel );
  }
  fprintf ( output, "\n" );*/
  

  rootpathrelations.reserve ( leg.depth + 2 );
  rightmostindexes.reserve ( leg.depth + 2 );
  if ( parenttree.rootpathstart == 0 && legindex >= parenttree.secondpathleg ) {
    // we're going from the first path to the second
    for ( i = 0; i < leg.depth; i++ ) {
      rootpathrelations.push_back ( 0 );
      rightmostindexes.push_back ( rootpathstart + i );
    }
    rootpathrelations.push_back ( treetuples[rootpathstart + i].edgelabel - leg.edgelabel );
    rightmostindexes.push_back ( treetuples.size () - 1 );
    if ( symmetric && treetuples[maxdepth + 1] == treetuples.back () )
      // in this case, we're copying the previous tree
      nextprefixindex = maxdepth + 2;
    else
      if ( treetuples[rootpathstart + leg.depth].edgelabel == leg.edgelabel )
	nextprefixindex = rootpathstart + leg.depth + 1;
      else
	nextprefixindex = NONEXTPREFIX;
  }
  else {
    ...
    if ( rootpathrelations.size () == 0 || rootpathrelations.back () ==  0 )
      rootpathrelations.push_back ( treetuples[rootpathstart + leg.depth].edgelabel - leg.edgelabel );
    else
      rootpathrelations.push_back ( rootpathrelations.back () );

    if ( parenttree.nextprefixindex != NONEXTPREFIX && 
         parenttree.treetuples[parenttree.nextprefixindex] == leg )
      nextprefixindex = parenttree.nextprefixindex + 1;
    else {
      if ( leg.depth < parenttree.rightmostindexes.size () &&
           parenttree.treetuples[parenttree.rightmostindexes[leg.depth]].edgelabel == leg.edgelabel )
        nextprefixindex = parenttree.rightmostindexes[leg.depth] + 1;
      else
        nextprefixindex = NONEXTPREFIX;
    }
  }
  ...
  if ( nextprefixindex == nextpathstart && symmetric == 1 ) {
    ...
    legmanager.appendExtensionLegs ( NODEPTH, graphstate.lastNode (), leg.connectingnode, MAXEDGELABEL );

    graphstate.subgraphiso = simplesubiso;
   
    tidlist = parenttree.tidlist->evaluateState();
  ...
  addExtensionLegs ( leg );

  int index = legindex;

  if ( nextprefixindex != NONEXTPREFIX && treetuples[nextprefixindex].depth <= leg.depth ) {
    Depth nextprefixdepth = treetuples[nextprefixindex].depth;
    EdgeLabel nextprefixlabel = treetuples[nextprefixindex].edgelabel;
    EdgeLabel lowestlabel = treetuples[rootpathstart + nextprefixdepth].edgelabel;
    while ( index < maxleg  &&
            parenttree.legs[index].depth > nextprefixdepth )
      index++;
    if ( index < maxleg && parenttree.legs[index].depth == nextprefixdepth &&
         nextprefixlabel != lowestlabel ) {
      if ( parenttree.legs[index].edgelabel == lowestlabel )
        index++;
      while ( index < maxleg &&
              parenttree.legs[index].depth == nextprefixdepth &&
	      parenttree.legs[index].edgelabel < nextprefixlabel )
	index++;
    }
  }
  ...
  if ( rootpathstart == 0 ) {
    ...
    while ( index < parenttree.legs.size () ) {
      ...
      legmanager.appendLeg ( parenttree.legs[index] );
      index++;
    }
    if ( index == parenttree.secondpathleg ) {
      legmanager.appendSecondPathMarker ( secondpathleg );
...
void PatternTree::expand () {
  ...
  if ( phase > 2 ) 
    for ( int i = 0; i < closelegs.size(); i++ ) {
      ...
      PatternGraph p ( closelegs, i, tidlist );
      p.expand ();
      graphstate.deleteEdge ( closelegs[i].from, closelegs[i].to );
    }
  
  for ( int i = legs.size () - 1; i >= 0; i-- ) {
    graphstate.insertNode ( legs[i].connectingnode, legs[i].edgelabel );
...
ostream &operator<< ( ostream &stream, Leg &tuple ) {
  DatabaseEdgeLabel edgelabel = database.edgelabels[database.edgelabelsindexes[tuple.edgelabel]];
  stream << "(" << tuple.depth << ","
         << database.nodelabels[edgelabel.fromnodelabel].inputlabel << "-"
         << edgelabel.inputedgelabel << "-"
         << database.nodelabels[edgelabel.tonodelabel].inputlabel << "[" << tuple.edgelabel << "])";
...
void PatternTree::checkIfIndeedNormal () {
  ...
  if ( !equal && treetuples[i-1].edgelabel > treetuples[j-1].edgelabel ) {
    cout << "NOT NORMAL: first path is higher than second" << endl;
  }
  for ( i = maxdepth + 1; i < nextpathstart; i++ ) {
    if ( treetuples[i].depth == maxdepth &&
         treetuples[i].edgelabel < treetuples[treetuples[i].depth].edgelabel )
  ...
  for ( i = nextpathstart + maxdepth + 1; i < treetuples.size (); i++ ) {
    if ( treetuples[i].depth == 0 ) {
      ...
      for ( ; j < treetuples.size () && equal; i++, j++ )
        equal = ( treetuples[i] == treetuples[j] );
      if ( !equal && treetuples[j-1].depth == maxdepth && treetuples[i-1].edgelabel > treetuples[j-1].edgelabel ) {
        cout << "NOT NORMAL: third path is lower than the second" << endl;
      }
      break;
    }
    if ( treetuples[i].depth == maxdepth && 
         treetuples[i].edgelabel < treetuples[treetuples[i].depth + nextpathstart].edgelabel )
